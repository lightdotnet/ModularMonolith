...
/** Requires `inventory.stock.view_cost` — the backend rejects the call otherwise. Same filter/paging params as `searchStockLevels`. */
export function getStockValuation(params: StockLevelSearchParams = {}) {
  return guardCall(() =>
    requestJson<Result<StockValuationDto>>("stock_level/valuation", {
      method: "GET",
      query: {
        productId: params.productId,
        locationId: params.locationId,
