...
/**
 * On-demand active-product picker for adding a purchase order line — never preloads the
 ...
 * `components/ui/*` primitive) — kept local to Purchasing rather than shared with
  ...
  React.useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      (async () => {
        ...
        if (cancelled) return;
        setLookupError(result.data ? null : (result.error ?? "Products could not be loaded."));
                ...
                <>
                  {lookupError ? (
                    <p className="px-3 py-4 text-sm text-muted-foreground">
                      {`Products could not be loaded. You may need Catalog access (catalog.products.view). ${lookupError}`}
                    </p>
                  ) : (
