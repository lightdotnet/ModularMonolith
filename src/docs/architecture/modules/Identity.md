...
- **Interactive server-rendered login** (`Identity.Web`) — an encrypted-cookie Razor Pages login for browser sessions, plus Microsoft Entra ID external login over OIDC.
...
- `TokenController` — routed at `api/v{version:apiVersion}/auth` (not a `token`-prefixed route). `auth/token/get` + `auth/token/refresh` are `[AllowAnonymous]`; `auth/token/hub` (`[Authorize]`) mints the SignalR handshake token via `IAuthenticationService.IssueHubTokenAsync` → returns `HubTokenResponse { AccessToken, ExpiresIn }`.
...
| `Infrastructure` | project (`Identity.Api → Infrastructure`, `Identity.Web → Infrastructure`) | `VersionedApiController`, `AppModule`/`AppModuleEndpoint`, shared infrastructure composition. |
