...
Note: `src/Migrations/MSSQL` also references `Location.Api` and `Catalog.Api` directly (for their
respective `DbContext`/`ContextInitialiser` pairs), same as every other module's MSSQL migrations
project — the Migrations projects are omitted from the diagram above, consistent with the rest of this
file. `Location` and `Catalog` are currently the only modules **not** also referenced from the
`PostgreSQL`/`Sqlite` migrations projects — see
...
`Location` and `Catalog` each have **no** outgoing cross-module `Contracts` dependency, and nothing yet
depends on either's `Contracts` project — their `ILocationDirectoryService`/`ICatalogPricingService`
seams have no current consumer (see [modules/Location.md](modules/Location.md) /
[modules/Catalog.md](modules/Catalog.md)). See the per-module docs for full detail.
