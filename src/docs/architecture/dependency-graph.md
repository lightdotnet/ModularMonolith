...
| Orders.Api | Lightsoft.AspNetCore.Authorization, Lightsoft.EntityFrameworkCore, Lightsoft.Mediator, Lightsoft.Result, Mapster | Every vendor package it directly uses is declared directly. `Mapster` is declared but currently unused (query handlers hand-map to DTOs instead). |
...
| Orders.Tests | xunit.v3, xunit.runner.visualstudio, Microsoft.NET.Test.Sdk, Moq | Mocks `ICatalogPricingService`/`ILocationDirectoryService` in the command-handler tests; else runs against a real Sqlite in-memory `OrdersDbContext`. |
...
Note: `src/Migrations/MSSQL` also references `Location.Api`, `Catalog.Api`, and `Orders.Api` directly
(for their respective `DbContext`/`ContextInitialiser` pairs), same as every other module's MSSQL
migrations project — the Migrations projects are omitted from the diagram above, consistent with the
rest of this file. `Location`, `Catalog`, and `Orders` are currently the only modules **not** also
referenced from the `PostgreSQL`/`Sqlite` migrations projects — see
...
Seven business-module-to-business-module dependencies exist, all compliant (each reaches only the other module's `Contracts` seam):
...
`Orders.Api` (the two edges above). See the per-module docs for full detail.
